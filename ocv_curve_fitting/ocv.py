...
temps = np.array([-5, 5, 15, 25, 35, 45, 55])

minV = 2.8     # minimum cell voltage, used for plotting results
maxV = 3.8     # maximum cell voltage, used for plotting results
...
for idx, temp in enumerate(temps):
    if temp < 0:
        ...
        files = [Path(f'../ocv_data/THUN100_OCV_N{tempfmt}_S1.csv'), Path(f'../ocv_data/THUN100_OCV_N{tempfmt}_S2.csv'),
                 Path(f'../ocv_data/THUN100_OCV_N{tempfmt}_S3.csv'), Path(f'../ocv_data/THUN100_OCV_N{tempfmt}_S4.csv')]
        data[idx] = BatteryData(files)
    else:
        tempfmt = f'{abs(temp):02}'
        files = [Path(f'../ocv_data/THUN100_OCV_P{tempfmt}_S1.csv'), Path(f'../ocv_data/THUN100_OCV_P{tempfmt}_S2.csv'),
                 Path(f'../ocv_data/THUN100_OCV_P{tempfmt}_S3.csv'), Path(f'../ocv_data/THUN100_OCV_P{tempfmt}_S4.csv')]
        data[idx] = BatteryData(files)
...
# discharge
indD = np.where(p25.s1.step == 6)[0]                            # slow discharge step
...
# charge
indC = np.where(p25.s3.step == 6)[0]                            # slow charge step
...
for k in not25:
    ...
    # discharge
    indD = np.where(data[k].s1.step == 6)[0]                                # slow discharge step
    ...
    # charge
    indC = np.where(data[k].s3.step == 6)[0]                             # slow charge step
...
nocv = len(filedata[3].rawocv)          # number of rawocv values based on 25 degC results
